...
export function OnboardingProfile({ navigation }: Props) {
  ...
  return (
    <OnboardingLayout step={2} title="Tu perfil físico" subtitle="Lo usaremos para calcular tus necesidades calóricas y de entrenamiento"
      onBack={() => navigation.goBack()} onContinue={() => { if (!validate()) return; setProfile({ gender: gender!, age: +age, weight: +weight, height: +height, bodyType: bodyType! }); navigation.navigate('OnboardingLifestyle'); }} canContinue={!!(gender && age && weight && height && bodyType)}>
      <Text style={s.label}>Género</Text>
      <View style={s.genderRow}>
        {([{value:'male',icon:'♂️',label:'Hombre'},{value:'female',icon:'♀️',label:'Mujer'},{value:'other',icon:'⚧',label:'Otro'}] as {value:Gender;icon:string;label:string}[]).map((g) => (
      ...
      {([{value:'slim',icon:'🪶',label:'Delgado/a',sublabel:'Dificultad para ganar peso'},{value:'athletic',icon:'🏃',label:'Atlético/a',sublabel:'Musculado/a, poca grasa'},{value:'average',icon:'🙂',label:'Promedio',sublabel:'Constitución media'},{value:'overweight',icon:'🍃',label:'Con sobrepeso',sublabel:'Quiero reducir grasa'}] as {value:BodyType;icon:string;label:string;sublabel:string}[]).map((b) => (
