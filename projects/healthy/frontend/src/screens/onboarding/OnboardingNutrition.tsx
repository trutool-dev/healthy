...
const RESTRICTIONS = [{id:'lactose',label:'Lactosa',icon:'🥛'},{id:'nuts',label:'Frutos secos',icon:'🥜'},{id:'shellfish',label:'Mariscos',icon:'🦐'},{id:'eggs',label:'Huevos',icon:'🥚'},{id:'soy',label:'Soja',icon:'🫘'},{id:'fish',label:'Pescado',icon:'🐟'}];
...
export function OnboardingNutrition({ navigation }: Props) {
  ...
  return (
    <OnboardingLayout step={5} title="Tu alimentación" subtitle="Creamos menús que disfrutes y se ajusten a tu forma de comer"
      onBack={() => navigation.goBack()} onContinue={() => { setNutrition({ dietType: diet!, restrictions: restr, budget: budget! }); navigation.navigate('OnboardingHealth'); }}
      canContinue={!!diet && !!budget}>
      <Text style={s.sec}>¿Cómo te alimentas?</Text>
      {([{value:'omnivore',icon:'🍗',label:'Omnívoro',sublabel:'Como de todo'},{value:'vegetarian',icon:'🥦',label:'Vegetariano',sublabel:'Sin carne ni pescado'},{value:'vegan',icon:'🌱',label:'Vegano',sublabel:'Solo productos vegetales'},{value:'gluten_free',icon:'🌾',label:'Sin gluten',sublabel:'Celíaco o intolerante'},{value:'keto',icon:'🥑',label:'Keto',sublabel:'Alta en grasas, baja en hidratos'},{value:'other',icon:'🍽️',label:'Otra',sublabel:''}] as {value:DietType;icon:string;label:string;sublabel:string}[]).map((d) => (
      ...
      <View style={s.tags}>
        {RESTRICTIONS.map((r) => (
          <Pressable key={r.id} onPress={()=>setRestr(p=>p.includes(r.id)?p.filter(x=>x!==r.id):[...p,r.id])} style={[s.tag, restr.includes(r.id) && s.tagSel]}>
      ...
      {([{value:'low',icon:'💰',label:'Ajustado',sublabel:'Menos de 150€/mes'},{value:'medium',icon:'💳',label:'Moderado',sublabel:'150 – 300€/mes'},{value:'high',icon:'💎',label:'Sin límite',sublabel:'Priorizo calidad'}] as {value:Budget;icon:string;label:string;sublabel:string}[]).map((b) => (
