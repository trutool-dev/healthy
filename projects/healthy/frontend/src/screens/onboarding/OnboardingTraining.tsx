...
export function OnboardingTraining({ navigation }: Props) {
  ...
  return (
    <OnboardingLayout step={4} title="Tu entrenamiento" subtitle="Diseñamos sesiones que encajen con tu tiempo y recursos"
      onBack={() => navigation.goBack()} onContinue={() => { setTraining({ daysPerWeek: days!, equipment, experience: experience! }); navigation.navigate('OnboardingNutrition'); }}
      canContinue={!!days && equipment.length > 0 && !!experience}>
      <Text style={s.sec}>Días disponibles por semana</Text>
      <View style={s.daysRow}>
        {[1,2,3,4,5,6,7].map((d) => (
          <Pressable key={d} onPress={()=>setDays(d)} style={[s.dayBtn, days===d && s.dayBtnSel]}>
            <Text style={[s.dayNum, days===d && s.dayNumSel]}>{d}</Text>
          </Pressable>
        ))}
      </View>
      {days && <Text style={s.daysHint}>{days === 1 ? '1 sesión' : `${days} sesiones`} a la semana</Text>}

      <Text style={[s.sec,{marginTop:spacing.lg}]}>Equipamiento disponible</Text>
      <View style={s.grid}>
        {([{value:'gym',icon:'🏋️',label:'Gimnasio'},{value:'home',icon:'🏠',label:'En casa'},{value:'outdoors',icon:'🌳',label:'Aire libre'},{value:'none',icon:'🤲',label:'Sin material'}] as {value:Equipment;icon:string;label:string}[]).map((e) => (
          <Pressable key={e.value} onPress={()=>toggleEq(e.value)} style={[s.equipCard, equipment.includes(e.value) && s.equipCardSel]}>
      ...
      {([{value:'beginner',icon:'🌱',label:'Principiante',sublabel:'Menos de 6 meses'},{value:'intermediate',icon:'🏃',label:'Intermedio',sublabel:'6 meses – 2 años'},{value:'advanced',icon:'🏆',label:'Avanzado',sublabel:'Más de 2 años'}] as {value:Experience;icon:string;label:string;sublabel:string}[]).map((e) => (
